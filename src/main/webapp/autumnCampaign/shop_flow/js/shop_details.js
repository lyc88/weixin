(function(){
  ...
  var details = {
      ...
      data_shop:function(){
          ...
          $.ajax({
              ...
              success:function(data){
                  ...
                  sessionStorage.setItem('ShopId',''+Prod.ruleList[0].shopId+'')
                  ...
                  //店铺渲染
                  function store_mes(){
                      $('.shops_store>div').eq(0).html('<img src="'+Src+ D.shopDetail.shopPic+'">')
                      $('.shops_store>div').eq(1).html('评论数('+ D.favoriteCount+')')
                  }
                  store_mes()
                  //banner 元素添加
                  function banner_img(){
                  ...
                  function _shop(){
                      ...
                      this.Message = function(i){
                          ...
                          Message.find('h4').html('￥'+Prod.cash)               //价格
                          Message.find('h5').html('库存'+Prod.stocks+'件')       //库存
                      }
                      this.Message()
                  ...
                  $('.go>a').click(function(){
                      ...
                      // if(SkuId){
                          Count = $('.number-on').val()
                          var mooncakeUserId = requestUrl('userId')
                          _this.addshopcart(ProdId,Count,mooncakeUserId,Index)
                          Tool.handle('操作成功',1)
                          setTimeout(function(){
                            Tool.handle('操作成功',-1)
                          },1000)
      ...
      //添加购物车
          //干掉了skuId addshopcart:function(prodId,sku_id,count,num){
          addshopcart:function(prodId,count,mooncakeUserId,num){
          console.log(prodId,count,num)
          $.ajax({
              'url':'http://song.85nv.cn/mobile/addShopCart',
              dataType:'Json',
              type:'post',
              // data:{prodId:prodId,sku_id:sku_id,count:count},
              data:{prodId:prodId,count:count,mooncakeUserId:mooncakeUserId},
              success:function(data){
                  console.log(data)
                  if(data.status==1){
                      if(num==0){
  ...
  details.tap($('.shopcar,#buyInMyStore'));
  details.tap($('.buy'));
