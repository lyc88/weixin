		...
		DateTimePicker.prototype = {
			...
			_setButtonAction: function()
			{
				...
				if(dtPickerObj.dataObject.oInputElement != null)
				{
					...
					dtPickerObj._hidePicker();
					var time = sOutput.split('-')
					var Year = time[0]
					var mouth = time[1]
					var Day = time[2]
					$.ajax(
						{url:'http://song.85nv.cn/mobile/p/updateBirthday',
							type:'post',
							data:{'year':Year,'month':mouth ,'day':Day},
							dataType:'Json',
			...
			_modifyPicker: function()
			{
				...
				sHeader += "<div class='dtpicker-title'>" + '请选择日期' + "</div>";
				...
				if(bDisplaySetButton)
					sDTPickerButtons += "<a class='dtpicker-button dtpicker-buttonSet'>" + '确定'+ "</a>";
				if(bDisplayClearButton)
					sDTPickerButtons += "<a class='dtpicker-button dtpicker-buttonClear'>" + '取消' + "</a>";
			...
			_setCurrentDate: function()
			{
				...
				if(dtPickerObj._compare(dtPickerObj.settings.mode, "date"))
				{
					...
					$(dtPickerObj.element).find('.day .dtpicker-compValue').val(sYear);
					$(dtPickerObj.element).find('.month .dtpicker-compValue').val(sMonthShort);
					$(dtPickerObj.element).find('.year .dtpicker-compValue').val(sDay);
				
					var sDate = sDayOfTheWeek + ", " + sYear + " " + sMonthFull + ", " +sDay ;
